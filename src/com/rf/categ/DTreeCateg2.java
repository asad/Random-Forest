...
import com.rf.fast.RandomForest;
...
//				copy.right=right.clone();
    ...
    /**
     * Evaluates each record and traverses through the tree and returns the
     * predictions
     *
...
//							System.out.println("going in child :"+evalNode.label);
     ...
     * this point.
     * </li>
     * <li>Step B Create a group of nodes and keep their references in node's
     * fields //Create a left and right node and keep their references in this
     ...
     * {@link #SortAtAttribute(List,int) sort} the data records by that
     * attribute , then look through the values from lowest to highest. If value
     * i is not equal to value i+1, record i in the list of "indicesToCheck."
     * This speeds up the splitting. If the number of indices in indicesToCheck
     * > MIN_SIZE_TO_CHECK_EACH then we will only
     ...
     * trying to find the minimum in which to split on
     * </li>
...
//			}
     ...
     * minimum.
     *
     * If the attribute is real-valued then : The total entropy is calculated by
     * getting the sub-entropy for below the split point and after the split
     * point. The sub-entropy is calculated by first getting the
     * {@link #GetClassProbs(List) proportion} of each of the classes in this
     * sub-data matrix. Then the entropy is
     ...
     *
     * If the attribute is categorical-valued then : The total entropy is
     * calculated by entropy of the each of categorical-value in the attribute
     * The split is done by recursively calling split over each of the
     * categorical value;
     *
     ...
     * representation, calculate an "entropy" value using the method in
     * Tan|Steinbach|Kumar's "Data Mining" textbook
     *
    ...
    /**
     * This class compares two data records by numerically/categorically
     * comparing a specified attribute
     *
     *
     */
